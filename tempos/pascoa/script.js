            ...
            function renderizarLeituras() {
                ...
                fetch("data/liturgiaAnoA.json")
                    ...
                    .then(data => {
                        ...
                        const chave = domingo.chaveLiturgia;

                        // 🔒 validação importante
                        if (!chave) {
                            console.error("chaveLiturgia não definida no JSON do dia");
                            listaCantosEl.innerHTML = "<p>Liturgia não configurada.</p>";
                            return;
                        }
                        ...
                        if (domingoLiturgia.primeiraLeitura) {
                            leituras.push({ titulo: "Primeira Leitura", dados: domingoLiturgia.primeiraLeitura });
                        }

                        if (domingoLiturgia.salmo) {
                            leituras.push({ titulo: "Salmo Responsorial", dados: domingoLiturgia.salmo });
                        }

                        if (domingoLiturgia.segundaLeitura) {
                            leituras.push({ titulo: "Segunda Leitura", dados: domingoLiturgia.segundaLeitura });
                        }


                        if (domingoLiturgia.sequencia) {
                            leituras.push({ titulo: "Sequência", dados: domingoLiturgia.sequencia });
                        }

                        if (domingoLiturgia.evangelho) {
                            leituras.push({ titulo: "Evangelho", dados: domingoLiturgia.evangelho });
                        }
